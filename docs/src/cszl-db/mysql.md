...
**优点**  
- 提高了数据查询效率，降低了`数据库的IO成本`
- 创建唯一索引可以保证表中每行`数据的唯一性`
- 使用分组和排序进行查询时，可以`减少查询中分组和排序的时间`，降低了CPU的消耗。

**缺点**    
- 创建的索引需要`占物理磁盘空间`，如果建立了大量的索引，索引文件可能比数据本身的文件会更快到达硬件限制。
- 索引会`降低更新表的速度`，当对表进行CRUD操作时，索引也需要动态更新，因此会影响更新表的整体速度。

:::info 最佳实践
当我们更新超大表数据时，最好的办法是先删除表中的索引，然后插入数据，插入成功后重建索引
:::
